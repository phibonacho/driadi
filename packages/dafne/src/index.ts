...
export type InnerNode<T> = Omit<INode<T>, "toRoot">;
const format = /^(?:(?<parent>\w+)\s)?(?<label>\w+)\s:\s(?<payload>.+)$/;
...
export default class Tree<T = string> implements ITree<T> {
	...
	commonAncestor(a: string, b: string): INode<T> | undefined {
		...
		if (aAncestors.some((ancestor) => ancestor === bNode)) return bNode;

		if (bAncestors.some((ancestor) => ancestor === aNode)) return aNode;

		const cursor =
			aAncestors.length <= bAncestors.length ? aAncestors : bAncestors;
		const test =
			aAncestors.length >= bAncestors.length ? aAncestors : bAncestors;
	...
	insert(node: InnerNode<T>, to?: string): INode<T> | undefined {
		if (!to) return this.insertTo(node, this.root);
		else {
			return this.insertTo(
				node,
				this.searchBreadthFirstByLabel(to, this.root)
			);
		}
	}
	...
	remove(label: string): INode<T> | undefined {
		...
		if (!toRemove || toRemove === this.root) return undefined;
		...
		if (parent.child === toRemove) {
			...
			if (!toRemove.sibling) parent.child = toRemove.child;
			else {
			...
			if (toRemove.child) this.insertTo(toRemove.child, previousSibling);
	...
	toString() {
		return JSON.stringify(this.nodeToString(this.root));
	}
	...
	static treeLit<P>(
		...
		const literal = strings.reduce((acc, val, index) => {
			if (parameters[index])
				return acc + val + parameters[index].toString();

			return acc + val;
		}, "");
		return Tree.parse(literal);
	}
	...
	static from<P>(source: ITree<P>): ITree<P> {
		const target = new Tree<P>();
	...
	private enumerateSub(node?: INode<T>): number {
		...
		return (
			(node === this.root ? 0 : 1) +
			this.enumerateSub(node.sibling) +
			this.enumerateSub(node.child)
		);
	}
	...
	private previousSibling({ child }: INode<T>, target: INode<T>): INode<T> {
		...
		while (cursor?.sibling) {
			if (cursor.sibling === target) break;
	...
	private searchBreadthFirstByLabel(
		target: string,
		current?: INode<T>
	): undefined | INode<T> {
		if (!current) return undefined;

		if (current.label === target) return current;

		return (
			this.searchBreadthFirstByLabel(target, current.sibling) ??
			this.searchBreadthFirstByLabel(target, current.child)
		);
	}
	...
	private nodeDegree(current?: INode<T>): number {
		...
		const degree: number =
			this.nodeDegree(current.child) + (current === this.root ? 0 : 1);

		return Math.max(siblingDegree, degree);
	}

	private nodeToString(node?: INode<T>): InnerNode<T> | undefined {
		if (!node) return undefined;

		const { label, child, sibling, payload } = node;
