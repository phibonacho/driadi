...
const siblingNode = {
	label: "sibling",
	payload: "this represent a sibling node",
};
const testNode = { label: "test", payload: "this is the node to test" };
const TREE_LITERAL = `parent : this represent a parent node
...
describe("Tree", () => {
	...
	describe("Tree.remove", () => {
		...
		it("should assign removed node's child to its first sibling when removed node is first child", () => {
			...
			assert.equal(
				removed.sibling?.child?.payload,
				removed.child?.payload
			);
		});
		...
		it("should assign removed node's child to its first sibling when removed node is not first child", () => {
			const previousNode = {
				...
				payload: "this represent a previous sibling node",
			};
	...
	describe("Tree.commonAncestor", () => {
		...
		it("should return root when no more specific ancestor is present", () => {
			...
			assert.equal(
				tree.commonAncestor(testNode.label, childNode.label),
				tree.root
			);
		});
		...
		it("should return undefined when one of the input nodes are not in tree", () => {
			...
			assert.equal(
				tree.commonAncestor(testNode.label, "notInTree"),
				undefined
			);
		});
		...
		it("should return the left operand when left operand is ancestor of right operand", () => {
			...
			assert.equal(
				tree.commonAncestor(testNode.label, parentNode.label),
				tree.search(parentNode.label)
			);
		});
	});
	...
	describe("Tree.from", () => {
		...
		it("should return a new tree from the one passed as parameter", () => {
			const newTree = Tree.from(tree);

			assert.equal(newTree.root.child?.label, tree.root.child?.label);
		});

		it("should return a new tree completely independet from the source one", () => {
			const newTree = Tree.from(tree);

			tree.remove(testNode.label);
			tree.insert(parentNode);

			assert.notEqual(newTree.root.child?.label, tree.root.child?.label);
		});
	});
	...
	describe("Tree.parse", () => {
		...
		it("should return a new tree from a valid configuration", () => {
			const parsedTree = Tree.parse(TREE_LITERAL);

			assert.equal(tree.toString(), parsedTree.toString());
		});
	});

	describe("Tree.treeLit", () => {
		let tree: Tree<string>;
		...
		it("should return a new tree from a valid literal", () => {
			const treeLit = Tree.treeLit`parent : this represent a parent node
parent child : this represent a child node
parent sibling : this represent a sibling node
`;

			assert.equal(tree.toString(), treeLit.toString());
		});
	});
});
